...
/*< init fdm utilities >*/
{
    ...
    fdm->nzpad= (free_) ? sf_n(az_)+fdm->nb : sf_n(az_)+2*fdm->nb ;
    fdm->nxpad= sf_n(ax_)+2*fdm->nb;

    fdm->ozpad= (free_) ? sf_o(az_) : sf_o(az_)-fdm->nb*fdm->dz;
...
*/
{
    ...
    abc->bzl = sf_floatalloc(fdm->nxpad+2);
    abc->bzh = sf_floatalloc(fdm->nxpad+2);
    abc->bxl = sf_floatalloc(fdm->nzpad+2);
    abc->bxh = sf_floatalloc(fdm->nzpad+2);
